...
| --- | --- | --- |
| Language | **TypeScript 7** (the native compiler), web and API. `openapi-typescript` alone runs on the `@typescript/typescript6` alias until it drops its compiler dependency — TS 7.0 ships no programmatic API (`06`, 2026-09-24) | Go, Java/Spring Boot; TypeScript 5 |
...
| Query layer | Drizzle ORM, over the same `pg` `Pool`, and Better Auth's Drizzle adapter over the same instance. The schema is Drizzle tables; drizzle-kit generates the SQL migrations from it (`04`, `12` §3). Chosen 2026-09-25 (`06`) | Kysely (the 2026-09-24 choice: its runner, dbmate, could not reach Neon), raw `pg` with hand-written row types |
| Lint and format | oxlint and oxfmt, pinned to exact versions (Oxc excludes type-aware rules from semver) | ESLint + typescript-eslint, Biome, Vite+ (revisit at 1.0) |
...
apps/api/               Hono app; `export default app` from src/index.ts (Vercel's Hono preset)
...
- **The API deploys on Vercel's Hono preset** (docs checked 2026-09-24): the app is the default export
  of `apps/api/src/index.ts`, every path reaches it, and no `vercel.json` routes are needed.
  `hono/vercel`'s `handle()` still exists but is no longer the documented path. Portability is
  unaffected: a default-exported Hono app is the thinnest adapter there is, and Node or Lambda entries
  wrap the same object. Hono's `serveStatic()` is ignored on Vercel; static files come from the CDN.
...
- Web Locks **in a standalone home-screen app**, and whether a lock is shared with the same origin in
  Safari (§8.1). The API itself is supported from Safari 15.4 — verified 2026-09-24.
...
### Verified 2026-09-24, and struck from the list above

Sources and reasoning in `06` (2026-09-24, platform facts).

| Was unverified | Answer |
| --- | --- |
| Web Locks in Safari | Supported since 15.4. Only the standalone-app question remains |
| `dbmate` inside Vercel's build image | The npm wrapper downloads nothing at install; `@dbmate/linux-x64` ships as an optional dependency. Migrations are transactional. A missing `pg_dump` silently skips the schema dump, which `--no-dump-schema` already avoids. **Moot since 2026-09-25:** its Go `lib/pq` refused Neon's SCRAM `i=1`, and drizzle-kit replaced it (`06`) |
| The `vercel.ts` rewrite syntax | `@vercel/config` 0.7.2, `routes.rewrite()`, named `config` export. One config file only — `vercel.ts` **or** `vercel.json` |
| How `@hono/zod-openapi` describes a multipart file part | Still open — deferred with S12 to M2 |
