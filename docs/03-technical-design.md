...
| Query layer | Kysely, over the same `pg` `Pool` handed to Better Auth. Types generated from the database, so they follow the migrations | Drizzle (its schema in TypeScript is a second source of truth, against `04`'s migrations-only rule), raw `pg` with hand-written row types |
...
| `dbmate` inside Vercel's build image | The npm wrapper downloads nothing at install; `@dbmate/linux-x64` ships as an optional dependency. Migrations are transactional. A missing `pg_dump` silently skips the schema dump, which `--no-dump-schema` already avoids |
